...
class InstantCSS
{
    ...
    public function icss_create_admin_sub_menu()
    {
        ...
        add_submenu_page(
            'instantcss', 'Instant CSS', 'Instant CSS',
            'manage_options', 'instantcss'
        );

        $customizer = add_submenu_page(
            'instantcss', 'Live editor', 'Live editor',
            'manage_options', 'instantcss-customizer', array( $this, 'icss_show_customizer_page' )
        );
    ...
    public function icss_admin_dependencies()
    {
        wp_enqueue_style(
            ...
            dirname(__FILE__)),
            array(),
            filemtime( plugin_dir_path( __FILE__ ) . 'assets/css/style.css' ) // Version: filemtime — Gets file modification time.);
        );
        wp_enqueue_script(
            'monaco-editor',
            plugins_url('assets/dist/main.bundle.js',
            dirname(__FILE__)),
            array(),
            filemtime( plugin_dir_path( __FILE__ ) . 'assets/dist/main.bundle.js' ), // Version: filemtime — Gets file modification time.
            true );
    ...
    public function icss_customizer_dependencies()
    {
        ...
        wp_enqueue_style( 'icss-customizer', plugins_url('assets/css/customizer.css', dirname(__FILE__)), array(), '1.1.3' );
    }
